...
if (!isset($_GET['id_course'])) {
    ...
    $is_join = JoinCourse::checkUserJoinCourse($user->id, $id_course);
    if ($is_join) {
        $cancel_btn = "d-inline";
        $register_btn = 'd-none';
    } else {
        $cancel_btn = "d-none";
        $register_btn = 'd-inline';
    }
...
<div class="d-flex flex-column col-xl-10 col-md-10 col-lg-10 p-0 content-wrapper px-5 pt-5 mt-2">
    <form action="index.php" method="GET" name="formCourseView">
        ...
        <div class='row justify-content-end mx-5 pr-2 pb-5'>
            <button type="button" class="w-25 mr-3 py-2 text-white w-600 btn red-bg <?= $cancel_btn ?>" data-toggle="modal" data-target="#exampleModal">CANCEL COURSE</button>
            <button type="button" class="w-25 mr-3 py-2 text-white w-600 btn red-bg <?= $register_btn ?>" onclick="registerCourse(<?= $id_course ?>)">REGISTER</button>
            <button type="button" class="px-5 py-2 w-600 btn btn-outline-secondary" onclick="back('home','schedule')">BACK</button>
        </div>
        <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Cancle course <?= $course->name ?></h5>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" onclick="cancelCourse(<?= $id_course ?>)" class="btn btn-primary">OK</button>
                    </div>
                </div>
            </div>
        </div>
